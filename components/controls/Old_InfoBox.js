...
const dropDownOptions = [
  ...
  'SV: Random Forest',
  'SV: XGBoost',
  'DT: Education Facility',
  'DT: Health Institution',
...
const dropDownDescriptions = [
  {
    ...
    desc: 'Hello and welcome to the DSVI Tool! This tool visualizes Social Vulnerability and Data Relevant for Social Vulnerability in Tajikistan. This is the info box.',
  },
...
const ControlMenu = (props) => {
  ...
  return (
    <>
        ...
        <div className="flex items-center border-black">
          ...
          >
            <svg
              ...
              className={`hover:bg-white-500 ml-2 h-11 w-11 cursor-pointer border-2 border-gray-600 bg-white bg-opacity-75 p-1 transition delay-150 duration-300 ease-in-out hover:scale-110 ${
      ...
      {props.children && (
        <Control position={position}>{props.children}</Control>
      )}
      ...
      {show_infoBox_data && (
        <Control minWidth={442} minHeight={360} position="topleft">
          <div ref={infoBoxRef} className="info-box">
            ...
            <Tabs />
            <Dropdown
              menuClassName="max-w-11/12 left-4p rounded-xl h-28"
              controlClassName="rounded-xl w-11/12 m-auto"
              options={dropDownOptions}
              onChange={(e) => changingDropdown(e.value)}
              value={dropdownValue}
              placeholder="Select an option"
            />
            <div className="mt-5 max-w-md px-4">
              <h2 className="font-bold">
                {dropDownDescriptions[dropdownDescIndex].heading}
              </h2>
              <p className="my-3">
                {dropDownDescriptions[dropdownDescIndex].desc}
              </p>
            </div>
          </div>
        </Control>
