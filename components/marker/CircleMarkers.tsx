  ...
  useEffect(() => {
    Papa.parse('static/demotool_data.csv', {
      ...
      complete: function (results, file) {
        const data = results.data.filter((point: any) => point.lat && point.lon)
  ...
  return (
    <>
      {dhsIndicator &&
        ...
        normalized_csv_data.map((point, index) => {
          ...
          return (
            <CircleMarker
              ...
              radius={2}
              pathOptions={{
                color: getColor(point[selected_dhs_column.title]),
              }}
            >
              <Popup>
                {selected_dhs_column.title} :{' '}
                {parseFloat(point[selected_dhs_column.title]).toFixed(2)}
              </Popup>
            </CircleMarker>
          )
        })}
